import
  ...
  btrees, idents, magicsys, extccomp, trees ]
...
template addSon*(c: PContext; p: PNode or PType; son: PNode) {.deprecated.} =
  ## a safe mutation of the node
  assert not c.isSealed, "attempt to mutate sealed context"
  if not c.isSealed:
    c.safeAdd son

proc setLocation*(c: PContext; p: PSym or PType; t: TLoc)
  {.tags: [TreeSafe, TreeRead, TreeWrite, RootEffect].} =
  ## set the location of a symbol|type in the given module
  assert p != nil, "nil node passed to setlocation"
  when p is PSym:
    let
      id = p.id
      k = nkSym
    ...
    {.fatal: "turn back!".}
  {.warning: "test that parent modules are the same".}
  assert not c.isSealed, "context is sealed"
  if not c.isSealed:
    p.setLocation(t)

template mutateLocation*(c: PContext; p: PSym or PType; body: untyped): untyped =
  if c.isSealed:
    raise newException(Defect, "tree is sealed")
  var
    loc {.inject.}: TLoc = p.loc
  body
  if loc != p.loc:
    if c.isSealed:
      raise newException(Defect, "tree is sealed")
...
when false:
  # ie. Indexable.add
  proc add*(c: PContext; father, son: PNode or PType) {.deprecated.} =
    assert son != nil
...
proc decodeLoc*(g; b; loc: var TLoc, info: TLineInfo) =
  if b.s[b.pos] == OpenLoc:
    ...
    if b.s[b.pos] == AnLiteral:
      inc(b.pos)
      loc.setRopeSecret(rope(decodeStr(b.s, b.pos)))
    else:
...
proc loadType*(g; id: int; info: TLineInfo): PType =
  ...
  result.mutateLocation:
    decodeLoc(g, b, loc, info)
          ...
          block search:
            for existing in g.cacheSeqs[destKey]:
              if exprStructuralEquivalent(existing, val, strictSymEquality=true):
                break search
            g.cacheSeqs[destKey].safeAdd val
      of "add":
        let destKey = n[1].strVal
        let val = n[2]
        if not contains(g.cacheSeqs, destKey):
...
proc loadNode*(g: ModuleGraph; module: PSym): PNode =
  ...
  for row in db.rows(sql"select data from toplevelstmts where module = ? order by position asc",
    ...
    # ensure we can read without index checks:
    b.s = row[0] & '\0'
    result.safeAdd decodeNode(g, b, module.info)
...
when true:
  ...
  template compileCachedIt*(context: PContext; p: PSym;
    ...
    context.performCachingOnIt(n, {Reads, Immutable}):
      body
